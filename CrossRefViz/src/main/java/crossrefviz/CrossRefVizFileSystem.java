...
@FileSystemInfo(type = "fstypegoeshere", // ([a-z0-9]+ only)
		description = "File system description goes here", factory = CrossRefVizFileSystem.MyFileSystemFactory.class)
public class CrossRefVizFileSystem implements GFileSystem {
	 ...
	 */
	public CrossRefVizFileSystem(FSRLRoot fsFSRL, ByteProvider provider) {
		this.fsFSRL = fsFSRL;
	...
	public void mount(TaskMonitor monitor) {
		monitor.setMessage("Opening " + CrossRefVizFileSystem.class.getSimpleName() + "...");
	...
	public static class MyFileSystemFactory
			implements GFileSystemFactoryByteProvider<CrossRefVizFileSystem>,
			GFileSystemProbeByteProvider {

		@Override
		public CrossRefVizFileSystem create(FSRLRoot targetFSRL,
				ByteProvider byteProvider, FileSystemService fsService, TaskMonitor monitor)
				throws IOException, CancelledException {

			CrossRefVizFileSystem fs = new CrossRefVizFileSystem(targetFSRL, byteProvider);
			fs.mount(monitor);
			return fs;
		}
