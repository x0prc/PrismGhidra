...
@FileSystemInfo(type = "fstypegoeshere", // ([a-z0-9]+ only)
		description = "File system description goes here", factory = TaintAnalysisFileSystem.MyFileSystemFactory.class)
public class TaintAnalysisFileSystem implements GFileSystem {
	 ...
	 */
	public TaintAnalysisFileSystem(FSRLRoot fsFSRL, ByteProvider provider) {
		this.fsFSRL = fsFSRL;
	...
	public void mount(TaskMonitor monitor) {
		monitor.setMessage("Opening " + TaintAnalysisFileSystem.class.getSimpleName() + "...");
	...
	public static class MyFileSystemFactory
			implements GFileSystemFactoryByteProvider<TaintAnalysisFileSystem>,
			GFileSystemProbeByteProvider {

		@Override
		public TaintAnalysisFileSystem create(FSRLRoot targetFSRL,
				ByteProvider byteProvider, FileSystemService fsService, TaskMonitor monitor)
				throws IOException, CancelledException {

			TaintAnalysisFileSystem fs = new TaintAnalysisFileSystem(targetFSRL, byteProvider);
			fs.mount(monitor);
			return fs;
		}
