...
@FileSystemInfo(type = "fstypegoeshere", // ([a-z0-9]+ only)
		description = "File system description goes here", factory = AutoAPIMappingFileSystem.MyFileSystemFactory.class)
public class AutoAPIMappingFileSystem implements GFileSystem {
	 ...
	 */
	public AutoAPIMappingFileSystem(FSRLRoot fsFSRL, ByteProvider provider) {
		this.fsFSRL = fsFSRL;
	...
	public void mount(TaskMonitor monitor) {
		monitor.setMessage("Opening " + AutoAPIMappingFileSystem.class.getSimpleName() + "...");
	...
	public static class MyFileSystemFactory
			implements GFileSystemFactoryByteProvider<AutoAPIMappingFileSystem>,
			GFileSystemProbeByteProvider {

		@Override
		public AutoAPIMappingFileSystem create(FSRLRoot targetFSRL,
				ByteProvider byteProvider, FileSystemService fsService, TaskMonitor monitor)
				throws IOException, CancelledException {

			AutoAPIMappingFileSystem fs = new AutoAPIMappingFileSystem(targetFSRL, byteProvider);
			fs.mount(monitor);
			return fs;
		}
